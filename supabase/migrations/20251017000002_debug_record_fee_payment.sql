-- Add debugging to the record_fee_payment function to understand why it's not finding fees
...
BEGIN
  ...
  -- Log debug info (this will appear in the error message if we fail)
  RAISE NOTICE 'DEBUG: Searching for fee with ID % and tenant_id %', p_fee_id, v_tenant_id;
  ...
  IF NOT FOUND THEN
    ...
    BEGIN
      ...
      IF v_fee_any_tenant IS NOT NULL THEN
        RAISE EXCEPTION 'Fee found but tenant mismatch. Fee tenant_id: %, User tenant_id: %. Debug info: %',
          v_fee_any_tenant.tenant_id, v_tenant_id, v_debug_info;
      ELSE
...
-- Add comment
COMMENT ON FUNCTION record_fee_payment IS 'Records payment for association fees with debugging';