-- Fix the tenant_id extraction from JWT token
-- The tenant_id is stored in app_metadata, not at the root level of the JWT

-- Drop the existing function to recreate with correct JWT extraction
DROP FUNCTION IF EXISTS record_fee_payment(UUID, DECIMAL(10,2), DATE, TEXT);

-- Create the function with CORRECT JWT tenant_id extraction
...
DECLARE
  ...
  v_debug_info JSONB;
BEGIN
  -- CORRECTED: Extract tenant_id from app_metadata in JWT token
...
COMMENT ON FUNCTION record_fee_payment IS 'Records payment for association fees with CORRECT JWT tenant_id extraction from app_metadata and household-tenant relationship JOIN logic.';